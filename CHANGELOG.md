...
- `fastjson_last_error_pos(): int` and `fastjson_last_error_info(): array`: surface the source location of the most recent parse error. yyjson already computes the byte offset of a read failure; fastjson now retains it (previously discarded) and maps it to a 1-based line/column via yyjson's `yyjson_locate_pos`. `fastjson_last_error_pos()` returns the byte offset (or `-1` when the last call succeeded or the error has no source position: encode, file I/O, and depth errors). `fastjson_last_error_info()` folds `fastjson_last_error()`, `fastjson_last_error_msg()`, and the location into one call: `['code'=>int, 'msg'=>string, 'pos'=>int, 'line'=>int, 'col'=>int]`. The location is captured at every read-error site (`fastjson_decode`, `fastjson_file_decode`, `fastjson_validate`, `fastjson_pointer_*`, `fastjson_merge_patch`).
- `fastjson_pointer_exists(string $json, string $pointer, int $flags = 0): bool`: report whether an [RFC 6901](https://www.rfc-editor.org/rfc/rfc6901) JSON Pointer resolves, without materializing anything into PHP. Unlike `fastjson_pointer_get()`, the bool result disambiguates "present but JSON null" (true) from "absent" (false). A false return with `fastjson_last_error() == FASTJSON_ERROR_NONE` means absent; a JSON parse error returns false with the error set, or throws under `JSON_THROW_ON_ERROR`. `$flags` carries the parse-affecting bits (`FASTJSON_DECODE_RELAXED`, `JSON_THROW_ON_ERROR`).
- `fastjson_pointer_set(string $json, string $pointer, mixed $value, int $flags = 0, int $depth = 512): string|false`: set the value at an RFC 6901 JSON Pointer and return the re-serialized document. The edit is applied in a yyjson mutable doc (only `$value` is materialized from PHP, not the whole input), so a single edit on a large document avoids a full decode/re-encode round-trip. Missing parent objects are created; the empty pointer `""` replaces the whole document. Returns false (or throws under `JSON_THROW_ON_ERROR`) when `$json` fails to parse, `$value` cannot be encoded, or the pointer cannot resolve to a settable location (array-index gap or scalar mid-path). Output formatting follows the encode bits of `$flags` (`JSON_PRETTY_PRINT`, `JSON_UNESCAPED_SLASHES`, `JSON_UNESCAPED_UNICODE`); the parse bits apply to `$json`. The untouched portion is re-emitted by yyjson's writer, so the documented `fastjson_encode` divergences (large/scientific doubles, U+2028/U+2029) apply to the whole output.
